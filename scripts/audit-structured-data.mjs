...
const prohibited=new Set(['brand','manufacturer','offers','price','priceCurrency','availability','review','reviews','aggregateRating']);
...
for(const file of(await walk(dist)).filter(file=>file.endsWith('.html'))){
  ...
  const types=schemas.map(item=>item?.['@type']).filter(Boolean);
...
else console.log('Structured data audit: OK (JSON parses, expected types and breadcrumb URLs are present, prohibited commercial fields: 0).');
